    ...
    <div class="pitstop-container py-8 sm:py-10">
        <section>
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h1 class="text-3xl font-bold tracking-tight text-blue-900">WELCUMMBEK ADMIN CiHUYERS</h1>
                    <p class="mt-1 text-sm text-slate-500">Lihat aktivitas booking dan layanan PitStop.</p>
                </div>
            ...
            <x-ui.card class="mt-3 flex items-center justify-between gap-4 p-4" data-testid="admin-stat-bookings-today"
                data-value="{{ $summary['bookings_today'] }}">
                <div>
                    <p class="text-sm font-semibold text-slate-700">Booking Hari Ini</p>
                    <p class="mt-1 text-xs text-slate-500">Jadwal service yang tercatat hari ini.</p>
                </div>
        ...
        <section class="mt-8">
            ...
            <div class="mt-4 grid gap-3 lg:grid-cols-3">
                @foreach ([[route('admin.services.index'), 'Kelola Layanan', 'Tambah dan perbarui layanan bengkel.'], [route('admin.bookings.index'), 'Daftar Booking', 'Lihat booking yang menunggu atau sedang diproses.'], [route('admin.bookings.history'), 'Riwayat Booking', 'Lihat booking yang sudah selesai atau dibatalkan.']] as [$href, $title, $description])
                    <a href="{{ $href }}"
        ...
        <section class="mt-8">
            <div class="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">Booking terbaru dari pelanggan.</p>
                </div>
            ...
            @if ($latestBookings->isEmpty())
                <x-ui.empty-state class="mt-4" title="Belum ada booking"
                    description="Booking pelanggan terbaru akan tampil di sini." />
            @else
                            ...
                            @foreach ($latestBookings as $booking)
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-slate-600">
                                        {{ $booking->start_time->format('d M Y, H:i') }} WIB</td>
                                    <td class="px-4 py-3"><x-ui.badge
                                            :variant="$booking->statusBadgeVariant()">{{ $booking->statusLabel() }}</x-ui.badge></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </x-slot>
                            ...
                            <x-ui.mobile-data-card :title="$booking->booking_code">
                                ...
                                <div class="col-span-2">
                                    ...
                                    <p class="mt-1 font-medium text-slate-700">
                                        {{ $booking->start_time->format('d M Y, H:i') }} WIB</p>
                                </div>
        ...
        <section class="mt-8">
            <div class="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-500">Ringkasan layanan yang tersedia.</p>
                </div>
                <a href="{{ route('admin.services.index') }}"
                    class="text-sm font-semibold text-blue-600 hover:text-blue-700">Kelola Layanan</a>
            </div>

            @if ($latestServices->isEmpty())
                <x-ui.empty-state class="mt-4" title="Belum ada layanan"
                    description="Layanan bengkel akan tampil di sini." />
            @else
                        ...
                        <x-ui.card class="p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-2 text-sm text-slate-500">Rp
                                        {{ number_format($service->price, 0, ',', '.') }} ·
                                        {{ $service->duration_minutes }} menit</p>
                                </div>
                                <x-ui.badge
                                    :variant="$service->is_active ? 'active' : 'inactive'">{{ $service->is_active ? 'Aktif' : 'Nonaktif' }}</x-ui.badge>
                            </div>
                        </x-ui.card>
                    @endforeach
                </div>
